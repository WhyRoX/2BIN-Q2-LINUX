...
int main(int argc, char **argv){
  ...
  // Ouverture en écriture (éventuellement vidage/création) des fichiers
  ...
  while ((nbCharRd = read(0, bufRd, MAX+1))) { /* Boucle de lecture */
    ...
    if (bufRd[nbCharRd-1] == '\n'){ /* Ligne ne dépasse pas MAX caractères */
      ...
      if (isupper(bufRd[0]))
        fd = fd1; /* fd sur fichier argv[1] */
      else if (islower(bufRd[0]))
        fd = fd2; /* fd sur fichier argv{2] */
      else 
        lnc = true;
      ...
      while ((nbCharRd = read(0,bufRd,MAX+1)) > 0 && (bufRd[nbCharRd-1] != '\n'))
